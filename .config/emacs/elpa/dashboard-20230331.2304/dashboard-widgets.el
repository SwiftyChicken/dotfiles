...
(declare-function all-the-icons-material "ext:data-material.el")
...
(defcustom dashboard-set-version-info t
  "When non nil, version info will be displayed under the banner."
  :type 'boolean
  :group 'dashboard)
...
(defcustom dashboard-version-info
  (lambda ()
    (format  "GNU Emacs version %d.%d" emacs-major-version emacs-minor-version))
...
(defcustom dashboard-path-max-length 60
...
(defface dashboard-version-info
  '((t :inherit default))
  "Face used for the init info."
  :group 'dashboard)

(defface dashboard-init-info
  '((t :inherit default))
  "Face used for the init info."
  :group 'dashboard)
...
(defface dashboard-heading-icon
  '((t (:inherit 'dashboard-heading)))
  "Face used for widget headings icons."
  :group 'dashboard)
...
(defun dashboard-insert-heading (heading &optional shortcut icon)
  ...
  (when (and (dashboard-display-icons-p) dashboard-set-heading-icons)
    ...
    (insert (cond
             ((string-equal heading "Recent Files:")
              (all-the-icons-material (cdr (assoc 'recents dashboard-heading-icons))
                                     :height 1.2 :v-adjust -0.15 :face 'dashboard-heading-icon))
             ((string-equal heading "Bookmarks:")
              (all-the-icons-octicon (cdr (assoc 'bookmarks dashboard-heading-icons))
                                     :height 1.2 :v-adjust 0.0 :face 'dashboard-heading-icon))
  ...
  (when shortcut (insert (propertize (format "              [%s]" shortcut) 'face '(:family "Fira Code Medium"
								      :foreground "white"
								      :height 190 
								      :weight demibold)
				     'display '(raise 0.15)))))
...
;; VERSION INFO
;;
(defun dashboard-insert-version-info ()
  "Insert version info when `dashboard-set-version-info' is t."
  (when dashboard-set-version-info
    (let ((version-info (if (functionp dashboard-version-info)
                       (funcall dashboard-version-info)
                       dashboard-version-info)))
      (dashboard-insert-center (propertize version-info 'face 'dashboard-version-info)))))
;;
;; INIT INFO
;;
(defun dashboard-insert-init-info ()
  ...
  (when dashboard-set-init-info
    (let ((init-info (if (functionp dashboard-init-info)
                       ...
                       dashboard-init-info)))
      (dashboard-insert-center (propertize init-info 'face 'dashboard-init-info)))))
    ...
    (dolist (line dashboard-navigator-buttons)
      (dolist (btn line)
        (let* ((icon (car btn))
               ...
               (icon-face (or (cadr (cddr (cddr btn))) 'dashboard-navigator))
               (prefix (or (cadr (cddr (cddr (cdr btn)))) (propertize "" 'face face)))
               (suffix (or (cadr (cddr (cddr (cddr btn)))) (propertize "" 'face face))))
          (widget-create 'item
                         :tag (concat
                               (when icon
                                 (propertize icon 'face
                                             (let ((prop-face (get-text-property 0 'face icon)))
                                               (if prop-face
                                                   `(:inherit ,prop-face :inherit ,icon-face)
                                                 `(:inherit ,icon-face)))))
                               (when (and icon title
